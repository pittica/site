...
export default class ServicesPostTemplate extends Component {
  render() {
    const post = this.props.data.graphCmsService;
    ...
    return (
      <PostLayout title={post.title} post={post} image={cover} location={this.props.location}>
        <article className="blog-post">
          ...
          {post.offers && (
            <Section title="Offerte" subtitle="Le offerte collegate a questo servizio">
...
export const pageQuery = graphql`
  query ServicesPostTemplate($slug: String!) {
    graphCmsService(stage: { eq: PUBLISHED }, locale: { eq: it }, slug: { eq: $slug }) {
      id
      title
      slug
      description
